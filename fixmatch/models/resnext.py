...
class ResNeXtBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, cardinality, base_width, widen_factor):
        ...
        if in_channels != out_channels:
            ...
            self.shortcut.add_module("shortcut_bn", nn.BatchNorm2d(out_channels, momentum=0.001))
...
class CifarResNeXt(nn.Module):
    ...
    def block(self, name, in_channels, out_channels, pool_stride=2):
        ...
        for bottleneck in range(self.block_depth):
            ...
            if bottleneck == 0:
                block.add_module(
                    name_, ResNeXtBottleneck(in_channels, out_channels, pool_stride, self.cardinality, self.base_width, self.widen_factor)
                )
            else:
                block.add_module(
                    name_, ResNeXtBottleneck(out_channels, out_channels, 1, self.cardinality, self.base_width, self.widen_factor)
                )
        return block
